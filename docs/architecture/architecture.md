...
### ADR 15: Cardio por METs + búsqueda de ejercicios + catálogo de comida peruana (F17)

- **Contexto:** Tres brechas sobre datos: (1) el cardio (correr/caminar/bici) SÍ existía en `training.exercises` (14 ejercicios `category='cardio'`) pero su gasto calórico era plano (6 cal/min × duración, sin distinguir tipo ni ritmo) y el peso del usuario ni se persistía; (2) "Agregar ejercicio" era una lista plana de 873 ítems sin buscador; (3) no había ningún dataset de comida (solo la caché vacía de OpenFoodFacts).
- **Decisión — cardio por METs:** se persiste `public.users.weight_kg` (el onboarding ya lo capturaba pero lo descartaba tras calcular el BMR) y se añaden `training.workout_sets.duration_min`/`distance_km` (nullable; fuerza los deja NULL, cardio los llena). El gasto se calcula con la fórmula estándar del Compendium `kcal = MET × peso_kg × horas`; para cardio el MET sale del ritmo real (`metFromSpeed(km/h)`, función pura con umbrales: caminar ~3.5–5, trotar ~7–8.3, correr ~9–11, correr rápido 11.5+), de modo que correr más rápido/más lejos da más kcal. La fuerza mantiene su estimación por duración de sesión (no cae a 0). Si el perfil no tiene peso, cae a 70 kg por defecto. `todayCaloriesBurned`/`caloriesBurnedForSessions` operan sobre las sesiones+sets ya cargados del día.
- **Decisión — búsqueda de ejercicios:** frontend puro sobre `provider.exercises` (ya en memoria, sin tocar DB): un buscador por nombre + `FilterChip` por `body_part` (derivados de los datos), filtrando por `body_part` o `secondary_muscles`; los `secondary_muscles` (ya poblados) se muestran como tags por ejercicio. Reusa la miniatura/detalle de F15.
- **Decisión — catálogo de comida (híbrido):** `nutrition.food_catalog` (lectura pública, mismo patrón que `training.exercises` — catálogo sin dueño, `GRANT SELECT` a anon/authenticated) sembrado con 50 platos peruanos curados (arroz con pollo, ceviche, lomo saltado, pollo a la brasa, etc.); **los macros son estimaciones honestas por porción típica**, documentado en el `COMMENT` de la tabla, no valores de laboratorio. En el Diario, una vía "Buscar en catálogo" (`ilike` sobre `food_catalog`) que reusa el diálogo de borrador existente → `addFoodLog`. Convive con las vías ya existentes (manual, cámara IA de F9, escáner de barras de F16 vía OpenFoodFacts) — de ahí "híbrido": platos caseros peruanos del catálogo curado + productos envasados de OpenFoodFacts.
- **Verificación:** lectura pública de `food_catalog` con anon confirmada (`tests/e2e/test_auth_rls_e2e.sh` AC12-AC13, 14/14 PASS); `metFromSpeed`/`caloriesBurnedForSessions` con tests (30 min/5 km > 20 min/2 km, fuerza-sola no cae a 0); búsqueda de ejercicios por nombre/músculo y del catálogo de comida con widget tests. Suite completa de Flutter (14 archivos) en verde, `flutter build web` OK.
