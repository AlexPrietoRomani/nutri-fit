...
- **Contexto:** T14.1.1 (Recuperación de Contraseña) añadió el servicio `mailpit` y variables `GOTRUE_SMTP_*` al servicio `auth` en `docker-compose.yml`. `docker compose up -d mailpit auth` recreó el contenedor `nutri-fit-auth` (nueva IP en `nutrifit-network`) sin tocar `postgres` ni `gateway` — a diferencia de INC-009/010/014, aquí no hubo `down -v` de por medio, solo un cambio de variables de entorno en un servicio existente.
- **Síntoma:** `POST /auth/v1/signup` vía el gateway (`http://localhost:54321`) devolvía `502 Bad Gateway`. `docker logs nutri-fit-gateway` mostraba `connect() failed (111: Connection refused) while connecting to upstream ... upstream: "http://172.22.0.4:9999/signup"` — una IP que ya no correspondía al contenedor `auth` recién recreado.
- **Hipótesis:** (1) la config SMTP nueva rompió GoTrue al arrancar — descartada, `docker logs nutri-fit-auth` no mostraba ningún error de SMTP ni de otro tipo, solo `GoTrue API started on: 0.0.0.0:9999`; (2) el gateway tenía cacheada la IP vieja de `auth` tras el recreate — confirmada por el log de nginx mostrando explícitamente la IP `172.22.0.4` como destino del `connect() failed`.
- **Causa raíz:** Igual mecanismo de fondo que INC-009 (orden de arranque) pero disparador distinto: nginx resuelve el upstream `auth:9999` de `proxy_pass` a una IP concreta una sola vez (no usa `resolver` dinámico), así que **cualquier** recreate del contenedor `auth` — no solo un `down -v` completo — deja al gateway con una IP obsoleta hasta que se reinicia. Confirma la sospecha ya anotada en INC-009: el problema no es exclusivo de "auth no existía al arrancar gateway", sino de cualquier cambio de IP de un upstream nombrado mientras gateway sigue corriendo.
- **Resolución:** `docker compose restart gateway` después de que `auth` quedara arriba. Ningún cambio de código adicional — el mailer de GoTrue quedó bien configurado a la primera (`GOTRUE_SMTP_HOST=mailpit`, `GOTRUE_SMTP_PORT=1025`, `GOTRUE_SMTP_ADMIN_EMAIL=no-reply@nutrifit.local`, sin variables extra de `GOTRUE_MAILER_URLPATHS_*` — el mailer se activa automáticamente al detectar `GOTRUE_SMTP_HOST` no vacío, confirmado tanto por los logs limpios como por la doc oficial de `supabase/auth`).
- **Verificación:** Tras el restart, `POST /auth/v1/signup` (200), `POST /auth/v1/recover` (200) y `POST /auth/v1/token?grant_type=password` (login, 200) respondieron correctamente vía el gateway. `curl http://localhost:8025/api/v1/messages` mostró el correo de recuperación real capturado en Mailpit (asunto "Reset Your Password", remitente `Nutri-Fit <no-reply@nutrifit.local>`, cuerpo con link `http://localhost:54321/verify?token=...&type=recovery&redirect_to=...` y código OTP alternativo). `docker compose ps` mostró los 6 servicios (`postgres`, `backend`, `postgrest`, `auth`, `mailpit`, `gateway`) arriba y sanos.
- **Lecciones:** La regla ya anotada en INC-009/010 se generaliza: **cualquier** `docker compose up`/`restart`/rebuild que recree `auth` o `postgrest` (no solo `down -v`) requiere reiniciar `gateway` después, porque nginx no resuelve DNS dinámicamente para upstreams por nombre. Sigue pendiente (anotado también en INC-009) evaluar un `resolver 127.0.0.11` + variable en `proxy_pass` en `docker/gateway/nginx.conf` si este patrón sigue repitiéndose — ya van 4 ocurrencias (INC-009, INC-010, INC-014, INC-016).
